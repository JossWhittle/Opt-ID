...
class ValidateMagnetCutoutsBoundaryError(ValidateMagnetCutoutsErrorBase):
    ...
    def __str__(self):
        return f'cutout extends outside the size of the magnet: size {self.size}, ' \
...
class ValidateMagnetCutoutsOverlapError(ValidateMagnetCutoutsErrorBase):
    ...
    def __init__(self, cutouts : npt.NDArray[(2, 2, 3), npt.Float]):
        super().__init__()
    ...
    def __str__(self):
        return f'cutouts overlap their areas of influence: ' \
               f'cutout A bottom-left-near corner {self.cutouts[0, 0]} ' \
               f'top-right-far corner {self.cutouts[0, 1]}, ' \
               f'cutout B bottom-left-near corner {self.cutouts[1, 0]} ' \
               f'top-right-far corner {self.cutouts[1, 1]}, '
...
def validate_magnet_cutouts(cutouts : npt.NDArray[(typing.Any, 2, 3), npt.Float],
                            size : npt.NDArray[(3,), npt.Float]):
    """
    ...
    cutouts = validate_tensor(cutouts, shape=(None, 2, 3))
    size = validate_tensor(size, shape=(3,))

    for a_index, a_cutout in enumerate(cutouts):
        if np.any(a_cutout[0] >= a_cutout[1]):
            raise ValidateMagnetCutoutsSizeError(magnet_cutout=a_cutout)

        if np.any(a_cutout[0] < 0) or np.any(a_cutout[1] > size):
            raise ValidateMagnetCutoutsBoundaryError(magnet_cutout=a_cutout, size=size)

        a_centre = np.mean(a_cutout, axis=0)
        a_extent = (a_centre - a_cutout[0])

        for b_index, b_cutout in enumerate(cutouts[(a_index + 1):]):
            ...
            if np.all(np.abs(b_centre - a_centre) < (a_extent + b_extent)):
                raise ValidateMagnetCutoutsOverlapError(
                    cutouts=cutouts[[a_index, (a_index + 1 + b_index)], ...])
